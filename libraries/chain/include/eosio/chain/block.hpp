#pragma once
#include <ETAio/chain/block_header.hpp>
...
FC_REFLECT_ENUM( ETAio::chain::transaction_receipt::status_enum,
                 (executed)(soft_fail)(hard_fail)(delayed)(expired) )

FC_REFLECT(ETAio::chain::transaction_receipt_header, (status)(cpu_usage_us)(net_usage_words) )
FC_REFLECT_DERIVED(ETAio::chain::transaction_receipt, (ETAio::chain::transaction_receipt_header), (trx) )
FC_REFLECT_DERIVED(ETAio::chain::signed_block, (ETAio::chain::signed_block_header), (transactions)(block_extensions) )

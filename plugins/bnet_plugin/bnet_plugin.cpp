...
#include <ETAio/bnet_plugin/bnet_plugin.hpp>
...
using ETAio::public_key_type;
using ETAio::chain_id_type;
using ETAio::block_id_type;
using ETAio::block_timestamp_type;
using std::string;
using ETAio::sha256;
using ETAio::signed_block_ptr;
using ETAio::packed_transaction_ptr;
using std::vector;
...
namespace ETAio {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              ETA_THROW(block_validate_exception, "bad block" );
           }
        ...
        void on( const packed_transaction_ptr& p ) {
           ...
           if (!p) {
              ...
              ETA_THROW(transaction_exception, "bad transaction");
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           ETA_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         ETA_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });

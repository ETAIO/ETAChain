...
class simpletoken : public ETAio::contract {
   public:
      ...
      void transfer( account_name from, account_name to, uint64_t quantity ) {
         ...
         ETAio_assert( fromacnt.balance >= quantity, "overdrawn balance" );
   ...
   private:
      ...
      ETAio::multi_index<N(accounts), account> _accounts;
      ...
      void add_balance( account_name payer, account_name to, uint64_t q ) {
         ...
         if( toitr == _accounts.end() ) {
           ...
           _accounts.modify( toitr, 0, [&]( auto& a ) {
              a.balance += q;
              ETAio_assert( a.balance >= q, "overflow detected" );
           });
         }
      }
};

ETAIO_ABI( simpletoken, (transfer)(issue) )

...
public:
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "ETAio_global_state", data, abi_serializer_max_time );
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(ETAio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create ETAio.msig and ETAio.token
        create_accounts({N(ETAio.msig), N(ETAio.token), N(ETAio.ram), N(ETAio.ramfee), N(ETAio.stake), N(ETAio.vpay), N(ETAio.bpay), N(ETAio.saving) });

        // Set code for the following accounts:
        //  - ETAio (code: ETAio.bios) (already set by tester constructor)
        //  - ETAio.msig (code: ETAio.msig)
        //  - ETAio.token (code: ETAio.token)
        set_code_abi(N(ETAio.msig), ETAio_msig_wast, ETAio_msig_abi);//, &ETAio_active_pk);
        set_code_abi(N(ETAio.token), ETAio_token_wast, ETAio_token_abi); //, &ETAio_active_pk);

        // Set privileged for ETAio.msig and ETAio.token
        set_privileged(N(ETAio.msig));
        set_privileged(N(ETAio.token));

        // Verify ETAio.msig and ETAio.token is privileged
        const auto& ETAio_msig_acc = get<account_object, by_name>(N(ETAio.msig));
        BOOST_TEST(ETAio_msig_acc.privileged == true);
        const auto& ETAio_token_acc = get<account_object, by_name>(N(ETAio.token));
        BOOST_TEST(ETAio_token_acc.privileged == true);


        // Create SYS tokens in ETAio.token, set its manager as ETAio
        ...
        create_currency(N(ETAio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to ETAio.system
        issue(N(ETAio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        // Set ETAio.system to ETAio
        set_code_abi(config::system_account_name, ETAio_system_wast, ETAio_system_abi);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(ETAio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "ETAio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), ETAio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), ETAio_assert_message_exception);

...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(ETAio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(ETAio.token), ETAio_token_wast );
   c.set_abi( N(ETAio.token), ETAio_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(ETAio.token), N(create), N(ETAio.token), mutable_variant_object()
              ("issuer",       "ETAio" )
   ...
   cr = c.push_action( N(ETAio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "dan" )

...
killAll()
{
  programs/ETAio-launcher/ETAio-launcher -k 15
}

cleanup()
{
    rm -rf etc/ETAio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clETA/clETA get info)"
  verifyErrorCode "clETA get info"
...
# stand up nodETA cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodETA \"--plugin ETAio::wallet_api_plugin\" $launcherOpts
programs/ETAio-launcher/ETAio-launcher --nodETA "--plugin ETAio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clETA/clETA --port $port2 get block 1)"
    verifyErrorCode "clETA get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clETA/clETA create key)"
verifyErrorCode "clETA create key"
...
PASSWORD_INITA="$(programs/clETA/clETA wallet create --name inita)"
verifyErrorCode "clETA wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clETA/clETA wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clETA wallet import"
programs/clETA/clETA wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clETA wallet import"
programs/clETA/clETA wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clETA wallet import"
programs/clETA/clETA wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clETA wallet import"
...
ACCOUNT_INFO="$(programs/clETA/clETA create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clETA create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clETA/clETA get account testera)"
verifyErrorCode "clETA get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clETA/clETA transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clETA transfer"
   ...
   TRANS_INFO="$(programs/clETA/clETA --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clETA get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done

...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for ETAio reqauth action with alice's spending key
   chain.link_authority("alice", "ETAio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "ETAio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with ETAio reqauth
   chain.unlink_authority("alice", "ETAio", "reqauth");
   ...
   // Link authority for any ETAio action with alice's scud key
   chain.link_authority("alice", "ETAio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "ETAio", "first",  "reqauth");
...
try {
   ...
   // Creating account with ETAio. prefix with privileged account
   chain.create_account("ETAio.test1");

   // Creating account with ETAio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("ETAio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'ETAio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "ETAio", "ETAio.any", "reqauth" );
   chain.link_authority( "bob", "ETAio", "ETAio.any", "reqauth" );

   /// this should succeed because ETAio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = ETAio::chain::resource_limits::resource_usage_object;
   using by_owner = ETAio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link ETAio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };

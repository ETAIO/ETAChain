...
namespace ETAio {
   ...
   void set_blockchain_parameters(const ETAio::blockchain_parameters& params) {
      char buf[sizeof(ETAio::blockchain_parameters)];
      ETAio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(ETAio::blockchain_parameters& params) {
      char buf[sizeof(ETAio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      ETAio_assert( size <= sizeof(buf), "buffer is too small" );
      ETAio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            ETAio_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return ETAio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = ETAio::memory_heap.malloc(count*size);
...
{
   return ETAio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return ETAio::memory_heap.free(ptr);
}

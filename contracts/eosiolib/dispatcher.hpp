...
namespace ETAio {
    ...
    * ```
    *
    * For this to work the Actions must be derived from ETAio::contract
    *
    */
   ...
   bool dispatch( uint64_t code, uint64_t act ) {
      ...
      return ETAio::dispatch<Contract,SecondAction,Actions...>( code, act );
   }
    ...
    * @tparam T - The contract class that has the correponding action handler, this contract should be derived from ETAio::contract
...
// Helper macro for ETAIO_API
#define ETAIO_API_CALL( r, OP, elem ) \
   case ::ETAio::string_to_name( BOOST_PP_STRINGIZE(elem) ): \
      ETAio::execute_action( &thiscontract, &OP::elem ); \
      break;

// Helper macro for ETAIO_ABI
#define ETAIO_API( TYPE,  MEMBERS ) \
   BOOST_PP_SEQ_FOR_EACH( ETAIO_API_CALL, TYPE, MEMBERS )
 ...
 * To be able to use this macro, the contract needs to be derived from ETAio::contract
 * 
 ...
 * Example:
 * @code
 * ETAIO_ABI( ETAio::bios, (setpriv)(setalimits)(setglimits)(setprods)(reqauth) )
 * @endcode
 */
#define ETAIO_ABI( TYPE, MEMBERS ) \
extern "C" { \
   void apply( uint64_t receiver, uint64_t code, uint64_t action ) { \
      auto self = receiver; \
      if( action == N(onerror)) { \
         /* onerror is only valid if it is for the "ETAio" code account and authorized by "ETAio"'s "active permission */ \
         ETAio_assert(code == N(ETAio), "onerror action's are only valid from the \"ETAio\" system account"); \
      } \
      if( code == self || action == N(onerror) ) { \
         TYPE thiscontract( self ); \
         switch( action ) { \
            ETAIO_API( TYPE, MEMBERS ) \
         } \
         /* does not allow destructor of thiscontract to run: ETAio_exit(0); */ \
      } \
   } \
} \
 /// @}  dispatcher

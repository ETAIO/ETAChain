...
namespace ETAio_system {

class ETAio_system_tester : public TESTER {
public:

   ETAio_system_tester()
   : ETAio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   ETAio_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ N(ETAio.token), N(ETAio.ram), N(ETAio.ramfee), N(ETAio.stake),
               N(ETAio.bpay), N(ETAio.vpay), N(ETAio.saving), N(ETAio.names) });
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(ETAio.token) );
         abi_def abi;
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance("ETAio")  + get_balance("ETAio.ramfee") + get_balance("ETAio.stake") + get_balance("ETAio.ram") );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& ETAin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",ETAin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(ETAio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = ETAio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(ETAio.token), symbol_code, N(stat), symbol_code );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(ETAio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "ETAio", "ETAio.msig", core_from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "ETAio", "alice1111111", core_from_string("650000000.0000"), "ETAio" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::ETAio_code_name}, 1},
...
inline uint64_t M( const string& ETA_str ) {
   return core_from_string( ETA_str ).get_amount();
}

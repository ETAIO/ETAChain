/**
 *  @file
 *  @copyright defined in ETAio/LICENSE.txt
 */
...
{
   try {
      app().set_version(ETAio::nodETA::config::version);
      app().register_plugin<history_plugin>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "ETAio/nodETA/data" );
      app().set_default_config_dir(root / "ETAio/nodETA/config" );
      if(!app().initialize<chain_plugin, http_plugin, net_plugin, producer_plugin>(argc, argv))
         return INITIALIZE_FAIL;
      initialize_logging();
      ilog("nodETA version ${ver}", ("ver", app().version_string()));
      ilog("ETAio root is ${root}", ("root", root.string()));
      app().startup();
      app().exec();

...
namespace ETAio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(ETAio);
const static uint64_t null_account_name      = N(ETAio.null);
const static uint64_t producers_account_name = N(ETAio.prods);
...
const static uint64_t ETAio_auth_scope       = N(ETAio.auth);
const static uint64_t ETAio_all_scope        = N(ETAio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t ETAio_any_name = N(ETAio.any);
const static uint64_t ETAio_code_name = N(ETAio.code);
...
const static ETAio::chain::wasm_interface::vm_type default_wasm_runtime = ETAio::chain::wasm_interface::vm_type::binaryen;
...
} } } // namespace ETAio::chain::config

constexpr uint64_t ETA_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / ETAio::chain::config::percent_100;
}

template<typename Number>
Number ETA_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + ETAio::chain::config::percent_100 - ETAio::chain::config::percent_1)  / ETAio::chain::config::percent_100;
}

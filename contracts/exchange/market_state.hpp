...
namespace ETAio {
   ...
   struct margin_position {
      ...
      ETALIB_SERIALIZE( margin_position, (owner)(borrowed)(collateral)(call_price) )
   };

   typedef ETAio::multi_index<N(margins), margin_position,
           indexed_by<N(callprice), ETAio::const_mem_fun<margin_position, uint64_t, &margin_position::get_call> >
   > margins;
   ...
   struct loan_position {
      ...
      ETALIB_SERIALIZE( loan_position, (owner)(interest_shares) )
   };

   typedef ETAio::multi_index<N(loans), loan_position> loans;

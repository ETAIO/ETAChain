...
class Cluster(object):
    ...
    # walletd [True|False] Is kETAd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="ETAtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kETAd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: ETA server host
        port: ETA server port
        walletHost: ETA wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures ETA mongo plugin
        ...
        self.ETAioAccount=self.defProducerAccounts["ETAio"]= Account("ETAio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontKill=False
               , dontBootstrap=False, totalProducers=None, extraNodETAArgs=None):
        """Launch cluster.
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.ETAServerName))
            return True
        ...
        if not Cluster.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodETA default port.")
            return False

        cmd="%s -p %s -n %s -s %s -d %s -i %s -f --p2p-plugin %s %s" % (
            Utils.ETALauncherPath, pnodes, totalNodes, topo, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            p2pPlugin, producerFlag)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodETAArgs="--max-transaction-time 50000 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodETAArgs += " --plugin ETAio::wallet_api_plugin"
        if self.enableMongo:
            nodETAArgs += " --plugin ETAio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodETAArgs is not None:
            assert(isinstance(extraNodETAArgs, str))
            nodETAArgs += extraNodETAArgs
        if Utils.Debug:
            nodETAArgs += " --contracts-console"

        if nodETAArgs:
            cmdArr.append("--nodETA")
            cmdArr.append(nodETAArgs)
        ...
        if nodes is None or totalNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.ETAServerName, totalNodes, len(nodes)))
            return False
        ...
        self.ETAioAccount=self.defProducerAccounts["ETAio"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.ETAClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.ETAClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on ETA server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextETAIdx: %d, n: %d" % (nextETAIdx, n))
                nextETAIdx=(nextETAIdx + 1)%count
                if not self.nodes[nextETAIdx].killed:
                    #Utils.Print("nextETAIdx: %d" % (nextETAIdx))
            ...
            #Utils.Print("nextETAIdx: %d, count: %d" % (nextETAIdx, count))
            node=self.nodes[nextETAIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.ETAServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on ETA node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getETABalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.ETAioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseProducers(nodeNum):
        ...
        configFile="etc/ETAio/%s/config.ini" % (node)
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse ETAio private keys from cluster config files.")
            return None
    ...
    def bootstrap(totalNodes, prodCount, biosHost, biosPort, dontKill=False, onlyBios=False):
        ...
        try:
            ...
            ETAioName="ETAio"
            ETAioKeys=producerKeys[ETAioName]
            ETAioAccount=Account(ETAioName)
            ETAioAccount.ownerPrivateKey=ETAioKeys["private"]
            ETAioAccount.ownerPublicKey=ETAioKeys["public"]
            ETAioAccount.activePrivateKey=ETAioKeys["private"]
            ETAioAccount.activePublicKey=ETAioKeys["public"]

            if not walletMgr.importKey(ETAioAccount, ignWallet):
                Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (ETAioName))
                return None
            ...
            trans=biosNode.publishContract(ETAioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
            if trans is None:
            ...
            for name, keys in producerKeys.items():
                ...
                trans=biosNode.createAccount(initx, ETAioAccount, 0)
                if trans is None:
            ...
            if not onlyBios:
                if prodCount == -1:
                    ...
                    with open(setProdsFile, "r") as f:
                        ...
                        myTrans=biosNode.pushMessage("ETAio", "setprods", setProdsStr, opts)
                    ...
                    trans=biosNode.pushMessage("ETAio", "setprods", setProdsStr, opts)
                ...
                # wait for block production handover (essentially a block produced by anyone but ETAio).
                lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "ETAio"
                ret=Utils.waitForBool(lam)
                if not ret:
                    Utils.Print("ERROR: Block production handover failed.")
                    return None

            ETAioTokenAccount=copy.deepcopy(ETAioAccount)
            ETAioTokenAccount.name="ETAio.token"
            trans=biosNode.createAccount(ETAioTokenAccount, ETAioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (ETAioTokenAccount.name))
                return None

            ETAioRamAccount=copy.deepcopy(ETAioAccount)
            ETAioRamAccount.name="ETAio.ram"
            trans=biosNode.createAccount(ETAioRamAccount, ETAioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (ETAioRamAccount.name))
                return None

            ETAioRamfeeAccount=copy.deepcopy(ETAioAccount)
            ETAioRamfeeAccount.name="ETAio.ramfee"
            trans=biosNode.createAccount(ETAioRamfeeAccount, ETAioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (ETAioRamfeeAccount.name))
                return None

            ETAioStakeAccount=copy.deepcopy(ETAioAccount)
            ETAioStakeAccount.name="ETAio.stake"
            trans=biosNode.createAccount(ETAioStakeAccount, ETAioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (ETAioStakeAccount.name))
                return None
            ...
            contract="ETAio.token"
            contractDir="contracts/%s" % (contract)
            wasmFile="%s.wasm" % (contract)
            abiFile="%s.abi" % (contract)
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(ETAioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
            if trans is None:
                Utils.Print("ERROR: Failed to publish contract %s." % (contract))
                return None

            # Create currency0000, followed by issue currency0000
            contract=ETAioTokenAccount.name
            Utils.Print("push create action to %s contract" % (contract))
            action="create"
            data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (ETAioTokenAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push create action to ETAio contract.")
                return None
            ...
            action="issue"
            data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (ETAioAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push issue action to ETAio contract.")
                return None
            ...
            Utils.Print("Verify ETAio issue, Expected: %s" % (expectedAmount))
            actualAmount=biosNode.getAccountETABalanceStr(ETAioAccount.name)
            if expectedAmount != actualAmount:
                Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                            (expectedAmount, actualAmount))
                return None

            contract="ETAio.system"
            contractDir="contracts/%s" % (contract)
            wasmFile="%s.wasm" % (contract)
            abiFile="%s.abi" % (contract)
            Utils.Print("Publish %s contract" % (contract))
            ...
            action="transfer"
            for name, keys in producerKeys.items():
                data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (ETAioAccount.name, name, initialFunds, "init transfer")
                opts="--permission %s@active" % (ETAioAccount.name)
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (ETAioTokenAccount.name, name))
                    return None
    ...
    # Populates list of ETAInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=0):
        ...
        cmd="pgrep %s %s" % (pgrepOpts, Utils.ETAServerName)
        ...
        for i in range(0, totalNodes):
            ...
            if m is None:
                Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.ETAServerName, pattern))
                break
    ...
    # Kills a percentange of ETA instances starting from the tail and update ETAInstanceInfos state
    def killSomeETAInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.ETAServerName, killSignal))
    ...
    def dumpErrorDetails(self):
        fileName="etc/ETAio/node_bios/config.ini"
        Cluster.dumpErrorDetailImpl(fileName)
        fileName="var/lib/node_bios/stderr.txt"
        Cluster.dumpErrorDetailImpl(fileName)

        for i in range(0, len(self.nodes)):
            fileName="etc/ETAio/node_%02d/config.ini" % (i)
            Cluster.dumpErrorDetailImpl(fileName)
            fileName="var/lib/node_%02d/stderr.txt" % (i)
            Cluster.dumpErrorDetailImpl(fileName)

    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodETA instances. allInstances will kill all nodETA instances running on the system."""
        cmd="%s -k 9" % (Utils.ETALauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down ETA cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the ETA server
            cmd="pkill -9 %s" % (Utils.ETAServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down ETA cluster.")
    ...
    def cleanup(self):
        for f in glob.glob("var/lib/node_*"):
            shutil.rmtree(f)
        for f in glob.glob("etc/ETAio/node_*"):
            shutil.rmtree(f)

...
###############################################################
# nodETA_run_remote_test
#  Tests remote capability of the nodETA_run_test. Test will setup cluster and pass nodes info to nodETA_run_test. E.g.
#  nodETA_run_remote_test.py -v --clean-run --dump-error-detail
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodETA_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodETA", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killETAInstances, False, False, killAll, dumpErrorDetails)

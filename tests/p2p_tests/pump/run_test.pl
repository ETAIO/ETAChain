...
my $ETA_home = defined $ENV{ETA_HOME} ? $ENV{ETA_HOME} : getcwd;
my $ETAd = $ETA_home . "/programs/ETAd/ETAd";
my $ETAc = $ETA_home . "/programs/ETAc/ETAc";

my $nodes = defined $ENV{ETA_TEST_RING} ? $ENV{ETA_TEST_RING} : "1";
my $pnodes = defined $ENV{ETA_TEST_PRODUCERS} ? $ENV{ETA_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$ETA_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of ETAd instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"ETA6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = ETAio::producer_plugin\n";
        print $cfg "plugin = ETAio::chain_api_plugin\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$ETAc transfer ETA inita 10 >> ETAc.out 2>> ETAc.err`;
            $counter++;
            if ($counter % 1000 == 0) {

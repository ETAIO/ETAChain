...
# --kill-count <nodETA instances to kill>
...
try:
    ...
    Print("Stand up ETA wallet kETAd")
    walletMgr.killall(allInstances=killAll)
    walletMgr.cleanup()
    if walletMgr.launch() is False:
        errorExit("Failed to stand up kETAd.")

    accountsCount=total_nodes
    walletName="MyWallet"
    Print("Creating wallet %s if one doesn't already exist." % walletName)
    wallet=walletMgr.create(walletName, [cluster.ETAioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    ETAioAccount=cluster.ETAioAccount
    ...
    if not cluster.createAccounts(ETAioAccount):
    ...
    if cluster.killSomeETAInstances(killCount, killSignal) is False:
        errorExit("Failed to kill ETA instances")
    Print("nodETA instances killed.")
    ...
    if cluster.relaunchETAInstances() is False:
        errorExit("Failed to relaunch ETA instances")
    Print("nodETA instances relaunched.")
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killETAInstances, killETAInstances, keepLogs, killAll, dumpErrorDetails)

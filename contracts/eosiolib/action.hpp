...
namespace ETAio {
    ...
    *    int  c; //4
    *
    *    ETALIB_SERIALIZE( dummy_action, (a)(b)(c) )
    *  };
   ...
   struct permission_level {
      ...
      ETALIB_SERIALIZE( permission_level, (actor)(permission) )
   };
   ...
   struct action {
      ...
      ETALIB_SERIALIZE( action, (account)(name)(authorization)(data) )
      ...
      void send_context_free() const {
         ETAio_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         ETAio_assert( name == T::get_name(), "Invalid name" );
         ETAio_assert( account == T::get_account(), "Invalid account" );
...
::ETAio::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), ACTION_NAME>::call

#define INLINE_ACTION_SENDER2( CONTRACT_CLASS, NAME )\
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::ETAio::string_to_name(#NAME) )
 ...
 * @param NAME - The name of the action
 */
#define ACTION( CODE, NAME ) struct NAME : ::ETAio::action_meta<CODE, ::ETAio::string_to_name(#NAME) >

...
try:
    ...
    if walletMgr.launch() is False:
        ...
        errorExit("Failed to stand up ETA walletd.")

    testWalletName="test"
    Print("Creating wallet \"%s\"." % (testWalletName))
    testWallet=walletMgr.create(testWalletName, [cluster.ETAioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    # create accounts via ETAio as otherwise a bid is needed
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.ETAioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.ETAioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.ETAioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.ETAioAccount, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.ETAioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.ETAioAccount, waitForTransBlock=True, exitOnError=True)
    ...
    actualAmount=node.getAccountETABalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountETABalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    contractDir="contracts/ETAio.token"
    wasmFile="ETAio.token.wasm"
    abiFile="ETAio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getETAAccountFromDb(currencyAccount.name)
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get ETA account failed", raw=True)
    ...
    account=node.getETAAccount(defproduceraAccount.name, exitOnError=True)
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killETAInstances, killWallet, keepLogs, killAll, dumpErrorDetails)

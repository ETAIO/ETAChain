...
#define N(X) ETAio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<ETAio::chain::name> : private hash<uint64_t> {
      typedef ETAio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const ETAio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, ETAio::chain::name& check);
} // fc


FC_REFLECT( ETAio::chain::name, (value) )
